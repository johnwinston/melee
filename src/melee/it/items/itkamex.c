...
void it_802CA49C(Item_GObj* gobj)
{
    ...
    it_80279C48(gobj, ap);
    ip->xDD4_itemVar.kamex.timer = -1;
    ip->xDBC_itcmd_var4.flags.x0 = false;
    ip->xDD4_itemVar.kamex.x64 = 0;
    ip->xDD4_itemVar.kamex.x68 = 0.0f;
    ip->xDD4_itemVar.kamex.x6C = 0.0f;
...
void it_802CA6A0(Item_GObj* gobj)
{
    ...
    if (ip->xDD4_itemVar.kamex.timer == -1) {
        ip->xDD4_itemVar.kamex.timer = attrs->timer;
        ip->xDD4_itemVar.kamex.x68 = attrs->x18;
        ip->xDD4_itemVar.kamex.x6C = attrs->x1C;
    }
...
void itKamex_UnkMotion1_Phys(Item_GObj* gobj)
{
    ...
    if (ip->x40_vel.x) {
        ...
        {
            ip->x40_vel.x +=
                ip->xDD4_itemVar.kamex.x6C * ip->facing_dir;
        }
    }
    if (ip->ground_or_air == GA_Air) {
        it_80272860(gobj, ip->xCC_item_attr->x10_fall_speed,
...
void it_802CA8DC(Item_GObj* gobj)
{
    ...
    if (ip->xDBC_itcmd_var4.flags.x0) {
        it_802CAB10(gobj);
        ip->x40_vel.x = ip->xDD4_itemVar.kamex.x68 * -ip->facing_dir;
        ip->xDBC_itcmd_var4.flags.x0 = false;
    }
}
